...
\begin{enumerate}
    \item \textit{Harmonia}: Pode ser definida como sendo o agrupamento sonoro. As combinações das notas tocadas simultaneamente de forma coordenada compõem a harmonia.
...
Um BD normalmente armazena os metadados externamente, mas pode ser projetado para suportar abordagens para o armazenamento interno de metadados. Cada qual apresenta vantagens e desvantagens, como pode ser visto na Tabela \ref{tab:diferencasArmazenamentoMetadados}.
...
Esses algoritmos são descritos em mais detalhes na subseção \ref{conteudo}.
...
Em seguida, a tarefa executada no sistema QBH é propor ao usuário uma lista de músicas que melhor correspondam à consulta, adotando uma medida de dissimilaridade apropriada entre os arquivos. A similaridade entre as músicas no repositório e a consulta é calculada por meio do DTW, visto na subseção \ref{dtw}, que o torna particularmente adequado para recuperação em sistemas QBH. O processo pode ser visto na Figura \ref{fig:qbhProcesso}.
...
Como vimos anteriormente, um dado musical geralmente é representado por um conjunto de características extraídas do conteúdo da música, ou seja, um conjunto de características que descrevem numericamente o dado. 



O conteúdo do dado (ou uma faceta do conteúdo, como artista, álbum e título da música) é analisado por meio de técnicas baseadas em busca por conteúdo, que utilizam essas características para representar e indexar as informações embutidas no dado musical.

\citeonline{cardoso2004} define em seu trabalho que os modelos de recuperação por conteúdo considerados clássicos são:
\begin{itemize}
    \item Modelo Booleano: Formula buscas através de uma expressão booleana composta por termos ligados através dos operadores lógicos AND, OR e NOT. O resultado da busca são arquivos de áudio digital cuja representação satisfaz as restrições lógicas da expressão de busca \cite{ferneda2003}.
    \item Modelo Vetorial: Propõe um ambiente no qual é possível obter arquivos de áudio digital que respondem parcialmente a uma expressão de busca. Isto é feito através da associação de pesos tanto aos termos de indexação como aos termos da expressão de busca. Esses pesos são utilizados para calcular o grau de similaridade entre a expressão de busca formulada pelo usuário e cada um dos arquivos de áudio digital ordenados pelo grau de similaridade de cada arquivo em relação à expressão de busca \cite{ferneda2003}.
    \item Modelo Probabilístico: Tenta tratar o problema da recuperação de informação dentro da visão probabilística. Dada uma consulta de um usuário, há um conjunto de documentos que possui arquivos de áudio digital relevantes e não-relevantes. Tendo a descrição desse conjunto não se teria problema em recuperar esses arquivos. Contudo, considerando que as propriedades de um conjunto ideal de respostas não são conhecidas de imediato na consulta, tenta-se adivinhar quais seriam essas propriedades \cite{bonfim2006}.
\end{itemize}


Esses modelos preveem que um arquivo de áudio digital é descrito como um conjunto de palavras-chave e cada característica é usualmente um valor ou um conjunto de valores numéricos. Mais informações sobre os modelos propostos podem ser vistos no trabalho do \citeonline[p.39]{ferreira2015}.

%=== RONALDO: ACHO QUE A EXPLICACAO ACIMA NAO ESTA CLARA. SE AS CARACTERISITCAS SAO OS METADADOS (TITULO, AUTOR, ANO,...), ELES NAO SAO APENAS VALORES NUMERICOS. ACREDITO QUE ELES SE TORNAM VALORES NUMERICOS QUANDO SAO MAPEADOS PARA O ESPACO VETORIAL. EXPLICAR MELHOR ESSE PARÁGRAFO ACIMA.

A partir dos modelos propostos nos sistemas de recuperação por conteúdo, as operações de comparação entre dados complexos utilizam os vetores de características para medir a similaridade do conteúdo presente nos dados que eles representam.

%=== RONALDO: FALTA AQUI UMA INTRODUCAO PARA OS ITENS DESCRITOS ABAIXO. O QUE ELES SAO? SAO AS OPERACOES DE COMPARACAO MENCIONADAS ACIMA? DEIXAR ISSO MAIS CLARO...
%=== GISELE: FEITO.

Desta forma, podemos verificar alguns algoritmos citados na literatura que são utilizados para medir a similaridade do conteúdo:
\begin{enumerate}
    ...
    Em um plano que contém os pontos \({P_{1}}\) e \({P_{2}}\), com as coordenadas \({(x_{1},y_{1})}\) e \({(x_{2},y_{2})}\) respectivamente, essa distância é definida através da Equação \ref{manhattan}.
    ...
    Sendo que o pdf é dado pela função \({f \colon R \rightarrow [0,1]}\) tal que \({\int_{R}f(x)dx = 1}\), e a função de distribuição para o pdf é definida através da Equação \ref{definitionFunction}.
...
Indexação é um processo pelo qual as características extraídas dos dados complexos (como artista, álbum e título da música) são armazenados em uma estrutura de índice para facilitar e acelerar a tarefa de busca dos dados musicais. Na literatura podem ser encontradas variadas formas de indexação para dados musicais. Portanto, o objetivo desta subseção é descrever de forma genérica como funcionam os métodos de indexação e citar os mais conhecidos para indexação de dados musicais.

%=== RONALDO: ESTÁS USANDO A SIGLA SGBD NO PARAGRAFO ABAIXO. ELA JA FOI USADA ANTES NO TEXTO? SE NAO FOI, TENS QUE DEFINI-LA AQUI POR EXTENSO PRIMEIRO.
%=== GISELE: JÁ FOI DEFINIDA NA LINHA 215, SEÇÃO "ARMAZENAMENTO DA INFORMAÇÃO MUSICAL"

Estruturas de indexação são normalmente fornecidas pelos SGBDs. Embora essas estruturas sejam suficientes para suprir as necessidades dos usuários de aplicações que lidam com dados convencionais, elas não são adequadas para os sistemas de recuperação de dados complexos, que lidam com dados que apresentam alta dimensionalidade e não apresentam relação de ordem \cite{barioni2006}.
