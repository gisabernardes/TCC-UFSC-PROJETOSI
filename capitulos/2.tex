...
A área de RIM conta com profissionais das mais diversas áreas inclusas na questão do tratamento e recuperação da informação musical como apresentado na tabela \ref{tab:comunidadeRim}, traduzida por \citeonline[p.11]{santini&souza2007}
...
\subsubsection{Audio Fingerprinting}
...
Esse sistema pode ser separado em dois processos: extração do \textit{fingerprint} (frontend), e o algoritmo de comparação (match). Podendo ser divido em oito etapas, conforme mostrado na Figura \ref{fig:etapasFinger}.
...
Segundo \citeonline{cano2005}, o pré-processamento se dá pela remoção dos dados que não interessam à extração de características, convertendo o sinal de entrada para o formato de 16 bits PCM\footnote{Pulse-code modulation: método usado para representação digital de sinais analógicos.}, agrupando através da média os canais esquerdo e direito em uma taxa de amostragem variando de 5 a 44,1 kHz.

O enquadramento adapta a amostragem para o que será utilizado e \citeonline{cano2005} o definem que:
...
Desta forma, para se extrair características, precisa-se ter o áudio no domínio da frequência \cite{bunnell1996a}. Assim, são aplicadas transformadas que convertem esses sinais em função da frequência, para que o algoritmo de reconhecimento possa realizar uma melhor classificação dentro das características desejadas.

\citeonline{cano2005} citam transformadas que podem ser utilizadas para extrair características:
...
Na extração de características os dados obtidos pela transformada serão novamente transformados de modo que aumente a sua invariância a distorções do som. E para complementar os dados absolutos, no pós-processamento, as variações temporais podem ser adicionadas ao sinal processado, através das derivativas de primeira e segunda ordem. Até aqui os métodos podem ser classificados como o frontend da obtenção do \textit{fingerprint} \cite{cano2005}.
...
%Método encontrado no SoundHound%

\subsubsection{Recuperação por Conteúdo}
% Euclidiana, Manhattan, Longest Common Subsequence, Transformada de Fourier de Curta Duração (Short Time Fourier Transform – STFT)
%Método encontrado no Musixmatch, ainda verificar se tem um método utilizado além do fingerprint%
%Incluiu recuperação da informação de artista, álbum e título de música que pode ser encontrado no Spotify, Deezer e SoundCloud%

%As técnicas baseadas em busca por conteúdo utilizam caracterı́sticas extraı́das automaticamente dos dados complexos para representar e indexar as informações embutidas nesses dados. Cada caracterı́stica é usualmente um valor ou conjunto de valores numéricos e o conjunto de caracterı́sticas extraı́das é chamado vetor de caracterı́sticas. Nos sistemas de recuperação por conteúdo, as operações de comparação entre dados complexos utilizam os vetores de caracterı́sticas para medir a similaridade do conteúdo presente nos dados que eles representam.
